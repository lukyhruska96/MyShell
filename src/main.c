...
main(int argc, char *argv[]) {
    ...
    if(opts.command != NULL) {
        ...
        if(fd == -1) {
            if(errno == EACCES) {
                fprintf(stderr, "Unable to find or open specified file.\n");
            }
            else {
                fprintf(stderr, "Unhandled exception while opening the file.\n");
            }
            EXIT_CODE = 1;
            sh_exit();
        }
        ...
        char *buff_loc = buff;
        ssize_t buff_read;
        while((buff_read = read(fd, buff_loc, buff_size-(buff_loc-buff)))) {
            if(buff_read == -1) {
